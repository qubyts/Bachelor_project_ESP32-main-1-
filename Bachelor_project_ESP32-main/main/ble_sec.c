...
void blufi_dh_negotiate_data_handler(uint8_t *data, int len, uint8_t **output_data, int *output_len, bool *need_free)
{
	...
	if (blufi_sec == NULL) {
		BLE_ERROR("BLUFI Security is not initialized");
	...
	case SEC_TYPE_DH_PARAM_LEN:
		...
		if (blufi_sec->dh_param == NULL) {
			...
			BLE_ERROR("%s, malloc failed\n", __func__);
			return;
		}
		break;
	case SEC_TYPE_DH_PARAM_DATA:{
		if (blufi_sec->dh_param == NULL) {
			BLE_ERROR("%s, blufi_sec->dh_param == NULL\n", __func__);
		...
		if (ret) {
			BLE_ERROR("%s read param failed %d\n", __func__, ret);
		...
		if (ret) {
			BLE_ERROR("%s make public failed %d\n", __func__, ret);
		...
		ret = mbedtls_dhm_calc_secret( &blufi_sec->dhm,
				...
				myrand, NULL);
		if (ret) {
			BLE_ERROR("%s mbedtls_dhm_calc_secret failed %d\n", __func__, ret);
			btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
			return;
		}
		...
		if (ret) {
			BLE_ERROR("%s mbedtls_md5 failed %d\n", __func__, ret);
